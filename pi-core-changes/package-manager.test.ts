...
Content`,
				...
				symlinkSync(sharedExtensionsDir, join(agentDir, "extensions"), dirLinkType);
				symlinkSync(sharedSkillsDir, join(agentDir, "skills"), dirLinkType);
				symlinkSync(sharedPromptsDir, join(agentDir, "prompts"), dirLinkType);
				symlinkSync(sharedThemesDir, join(agentDir, "themes"), dirLinkType);
				symlinkSync(sharedExtensionsDir, join(tempDir, ".pi", "extensions"), dirLinkType);
				symlinkSync(sharedSkillsDir, join(tempDir, ".pi", "skills"), dirLinkType);
				symlinkSync(sharedPromptsDir, join(tempDir, ".pi", "prompts"), dirLinkType);
				symlinkSync(sharedThemesDir, join(tempDir, ".pi", "themes"), dirLinkType);

				const result = await packageManager.resolve();

				const sharedExtensions = result.extensions.filter((resource) => pathEndsWith(resource.path, "shared.ts"));
				const sharedSkills = result.skills.filter((resource) =>
					pathEndsWith(resource.path, join("shared-skill", "SKILL.md")),
				);
				const sharedPrompts = result.prompts.filter((resource) => pathEndsWith(resource.path, "shared.md"));
				const sharedThemes = result.themes.filter((resource) => pathEndsWith(resource.path, "shared.json"));
