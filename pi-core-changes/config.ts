...
function hasWriteModeBit(path: string): boolean {
...
function isSelfUpdatePathWritable(): boolean {
	...
	try {
		if (!hasWriteModeBit(packageDir) || !hasWriteModeBit(parentDir)) return false;
...
function getPackageDirInferredRoot(method: InstallMethod): string | undefined {
	const packageDir = getPackageDir();
	const path = process.platform === "win32" || packageDir.includes("\\") ? win32 : { basename, dirname };
	const parent = path.dirname(packageDir);
	if (!path.basename(parent).startsWith("@")) return undefined;
	const nodeModules = path.dirname(parent);
	if (path.basename(nodeModules) !== "node_modules") return undefined;
	if (method === "npm") {
		const rootParent = path.dirname(nodeModules);
		return path.basename(rootParent) === "lib" ? nodeModules : undefined;
	}
	return nodeModules;
}
...
function isManagedByGlobalPackageManager(method: InstallMethod, packageName: string, npmCommand?: string[]): boolean {
	...
	const roots = [...getGlobalPackageRoots(method, packageName, npmCommand), getPackageDirInferredRoot(method)].filter(
