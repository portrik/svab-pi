...
describe("SessionManager worktree metadata", () => {
	let tempDir: string;
	...
	it("stores worktree metadata in session header", () => {
		const worktreeMeta = {
			ownedBy: "pi" as const,
			originalCwd: "/home/user/project",
			gitRoot: "/home/user/project",
			worktreePath: "/tmp/pi-worktree-abc",
			relativeCwd: "",
		};
		const sm = SessionManager.create(tempDir, tempDir, { worktree: worktreeMeta });
		const header = sm.getHeader();
		expect(header?.worktree).toEqual(worktreeMeta);
	});

	it("preserves worktree metadata when loading session", () => {
		const worktreeMeta = {
			ownedBy: "pi" as const,
			originalCwd: "/home/user/project",
			gitRoot: "/home/user/project",
		...
		const sm1 = SessionManager.create(tempDir, tempDir, { worktree: worktreeMeta });
		sm1.appendMessage({ role: "user", content: "hello", timestamp: Date.now() });
		sm1.appendMessage({
			role: "assistant",
